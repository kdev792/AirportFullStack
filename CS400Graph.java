...
// Notes to Grader: <Used the graphs from the lecture videos as reference because I knew those were right. Also referred to Pseudo code provided in lecture.
// Also, utilized the CSLC and office hours with Professor Dahl and a few TA's to fix logic for the Djikstra's method>
...
public class CS400Graph<T> implements GraphADT<T> {
	...
	protected class Path implements Comparable<Path> {
		...
		public Path(Path source) {
			...
			// sets the source values as the local values
			this.start = source.start;
			this.distance = source.distance;
			this.dataSequence = new LinkedList<>();

			// loops through the sequence for the source node and adds them to the new local
			// dataSequence
			...
			// sets the parameter end as the local end
			this.end = source.end;
		}
		...
		public void extend(Edge edge) {
			...
			// adds the target vertex of the edge (and thus the edge) to the dataSequence
			dataSequence.add(edge.target.data);

			// increases the distance by adding the weight of the edge to it
			distance += edge.weight;

			// updates the end of the sequence to be the target vertex of the newly added
			// edge
			end = edge.target;
		}
	...
	protected Path dijkstrasShortestPath(T start, T end) throws NoSuchElementException {
		...
		// Makes both the start and end parameters into type Vertex to be used later
		Vertex vertexStart = vertices.get(start);
		Vertex vertexEnd = vertices.get(end);

		// LinkedList that will eventually store the visited vertices in the graph
		LinkedList<Edge> visited = null;

		// check to see if either the start or end vertex is null, if it is then the
		// method throws a NoSuchElementException
		if (vertexStart == null || vertexEnd == null) {
			throw new NoSuchElementException("No Shortest Path Found");
		}

		// Path type object to hold the
		Path toAdd = new Path(vertexStart);

		// Priority queue needed for the Dijkstra's algorithm
		PriorityQueue<Path> pq = new PriorityQueue<Path>();

		// Path type variable that will store the shortest path and be returned later
		Path toReturn = null;

		// Path for the 'c' value, the pointer to the current vertex in the queue
		Path current = null;

		// adding the Path type object with the start vertex to the priority queue
		pq.add(toAdd);

		// if the start vertex and the end vertex are the same then the shortest path is
		// just that vertex
		if (vertexStart == vertexEnd) {

			// This is for when there is a loop, where the start and end vertex are the same
			// but the weight isn't 0
			if (vertexStart.edgesLeaving.contains(vertexEnd.data)) {
				toAdd.distance = vertexStart.edgesLeaving.poll().weight;
			}

			return toAdd;
		}

		// loops through as long as the priority queue is not empty
		while (!pq.isEmpty()) {

			// removes the head of the PriorityQueue and stores it in the Path object
			// current
			current = pq.poll();

			// T type data variable to hold the data for end of the pointer path object
			T toCompare = current.end.data;

			// if the data from the pointer equals the end vertex and there is no shortest
			// path currently in place then the current vertex is the shortest path
			if (toCompare.equals(end) && toReturn == null) {
				toReturn = current;
			}

			// if the data from the pointer equals the end vertex but there IS a shortest
			// path currently in place then we check if the distance for the current pointer
			// vertex is less than the shortest path, if it is then the shortest path is
			// updated to the current
			else if (toCompare.equals(end) && toReturn != null && current.distance < toReturn.distance) {
				toReturn = current;
			}

			// otherwise the pointer is not at the end and we must add its successors and
			// check if they have been visited or not
			else {

				/**
				 * check out visited LinkedList
				 */
				// add the vertices of the edges leaving to an edge typed LinkedList that we
				// will use to check against
				visited = vertices.get(toCompare).edgesLeaving;

				// loop through the visited LinkedList to check
				for (int i = 0; i < visited.size(); i++) {

					// if the data sequence of the current path contains anything from the visited
					// LinkedList at that specified index then it will not enter the loop. If the
					// vertex has NOT been visited then it will enter the loop
					if (!(current.dataSequence.contains(visited.get(i).target.data))) {

						// this new Path object holds the current vertex value
						toAdd = new Path(current);

						// the weight of the edges, the distance, and the end vertex are all updated via
						// the extend helper method made above
						toAdd.extend(visited.get(i));

						// this path is then added to the priority queue, which sorts automatically
						pq.add(toAdd);
					}
				}
			}
		...
		// if the shortest path is still null after all this then an exception is thrown
		// because clearly the paths do not have a path between them
		if (toReturn == null) {
			throw new NoSuchElementException("No Shortest Path Found");
		}

		// returns the shortest path between the start and end data points
		return toReturn;
	}
